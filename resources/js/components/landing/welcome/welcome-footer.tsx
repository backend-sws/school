import { FOOTER_SECTIONS, IMPORTANT_LINKS, COLLEGE_DETAILS } from '@/constants';
            ...
            <div className="mx-auto max-w-[1440px] px-3 sm:px-4 md:px-6 lg:px-8 relative z-10">
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 sm:gap-10 md:gap-12 mb-10 sm:mb-14 md:mb-16 pb-10 sm:pb-14 md:pb-16 border-b border-border/60">
                    ...
                    <div className="md:col-span-4 space-y-4 sm:space-y-6">
                        <div className="flex items-center gap-3 sm:gap-5">
                            <div className="p-2.5 sm:p-3.5 rounded-lg sm:rounded-xl bg-muted border border-border transition-colors group-hover:border-primary/30 shrink-0">
                                <AppLogoIcon alt="" className="h-8 w-8 sm:h-10 sm:w-10 object-contain" />
                            </div>
                            <div className="min-w-0">
                                <h4 className="font-bold text-base sm:text-lg md:text-xl text-foreground leading-none tracking-tight">{name}</h4>
                                <p className="text-[10px] sm:text-[11px] text-primary font-bold mt-1.5 sm:mt-2 uppercase tracking-widest">{locationLine}</p>
                            </div>
                        </div>
                        <p className="text-xs sm:text-[13px] text-muted-foreground font-medium leading-relaxed max-w-sm">
                            {COLLEGE_DETAILS.description}
                        </p>
                    </div>
                            ...
                            <div key={section.title}>
                                ...
                                <ul className="space-y-2.5 sm:space-y-4 text-muted-foreground">
                                    {section.links.map((link) => (
                                        <li key={link.title}>
                                            <a href={link.href} className="group flex items-center gap-2 text-xs sm:text-[13px] font-bold hover:text-primary transition-colors">
                    ...
                    <div className="md:col-span-3 bg-muted/30 border border-border/60 p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl">
                        <h4 className="font-bold text-foreground text-[9px] sm:text-[10px] uppercase tracking-[0.15em] sm:tracking-[0.2em] mb-4 sm:mb-6 md:mb-8">Contact Us</h4>
                        <ul className="space-y-4 sm:space-y-6">
                            <li className="flex items-start gap-3 sm:gap-4">
                                ...
                                <span className="text-xs sm:text-[13px] font-bold text-foreground leading-relaxed pt-0.5 sm:pt-1">{contact.address}</span>
                            </li>
                            <li>
                                <a href={`tel:${(contact.phone || '').replace(/\s/g, '')}`} className="flex items-center gap-3 sm:gap-4 group">
                                    ...
                                    <span className="text-xs sm:text-[13px] font-bold text-foreground group-hover:text-primary transition-colors break-all">{contact.phone}</span>
                                </a>
                            </li>
                            <li>
                                <a href={`mailto:${contact.email}`} className="flex items-center gap-3 sm:gap-4 group">
                                    <div className="p-2.5 sm:p-3 rounded-lg bg-primary/10 text-primary border border-primary/20 shrink-0 group-hover:bg-primary group-hover:text-primary-foreground transition-colors">
                                        <Mail className="h-4 w-4 sm:h-4.5 sm:w-4.5" />
                                    </div>
                                    <span className="text-xs sm:text-[13px] font-bold text-foreground group-hover:text-primary transition-colors break-all">{contact.email}</span>
                                </a>
                            </li>
                        </ul>
                    </div>
                ...
                {/* Vital Portals Section */}
                <div className="mb-10 sm:mb-14 md:mb-16">
                    <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6 md:mb-8">
                        <span className="text-[9px] sm:text-[10px] font-bold text-primary uppercase tracking-[0.15em] sm:tracking-[0.2em]">Vital Portals</span>
                    </div>

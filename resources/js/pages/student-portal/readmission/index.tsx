...
import {
  STUDENT_READMISSION_FORM_FIELDS,
  STUDENT_READMISSION_FORM_INITIAL_DATA,
} from "@/constants/page/admin/studentReadmission";
import { StudentReAdmissionFormSchema } from "@/lib/validations/studentReadmission";
import FeePreview from "@/components/admin/feePreview";
...
const FORM_STEPS = [
  ...
  ADMISSION_FORM_STEP.FEE_PREVIEW,
];
...
const Admission = () => {
  ...
  const {
    ...
    resolver: zodResolver(StudentReAdmissionFormSchema) as any,
    defaultValues: STUDENT_READMISSION_FORM_INITIAL_DATA,
  });
  ...
  const instructionQuery = useQuery({
    queryKey: ["admissionInstruction", "admission_re_instruction"],
    queryFn: () => AdmissionApi.getSettingByKey("admission_re_instruction"),
    enabled: step === ADMISSION_STEP.INSTRUCTION,
  });

  const tcQuery = useQuery({
    queryKey: ["admissionTC", "admission_re_tc"],
    queryFn: () => AdmissionApi.getSettingByKey("admission_re_tc"),
    enabled: step === ADMISSION_STEP.INSTRUCTION,
  });

  const verifyMutation = useMutation({
    mutationFn: AdmissionApi.verifyReAdmissionId,
    onSuccess: () => {
  ...
  const basicDetailFields = STUDENT_READMISSION_FORM_FIELDS.filter(
    (field) => !field.name.startsWith("addresses."),
  );

  const permanentAddressFields = STUDENT_READMISSION_FORM_FIELDS.filter(
    (field) => field.name.startsWith("addresses.0"),
  );

  const correspondenceAddressFields = STUDENT_READMISSION_FORM_FIELDS.filter(
      ...
      <TooltipProvider>
        <div className="p-4 sm:p-6 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <Heading
              title="Re Admission Management"
              description="Manage Re Admission."
                  ...
                  <CardContent className="space-y-6">
                    {admissionFormQuery.isLoading ? (
                      ...
                      <>
                        ...
                        {formStep === ADMISSION_FORM_STEP.FEE_PREVIEW && (
                          <FeePreview

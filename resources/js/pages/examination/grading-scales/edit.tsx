...
interface GradingScaleEditProps {
  scale: Record<string, unknown> & { id: number; name: string; description?: string; rules?: Record<string, unknown>[] };
}

export default function GradingScaleEdit({ scale }: GradingScaleEditProps) {
  const { data, setData, put, processing, errors } = useForm<{
    ...
    rules: scale.rules?.length ? (scale.rules as { grade: string; min_percentage: number; max_percentage: number; grade_point: number; description: string; }[]) : [
  ...
  const handleSubmit: FormEventHandler = (e) => {
    e.preventDefault();
    put(`/examination/grading-scales/${scale.id}`);
  };
  ...
  const removeRule = (index: number) => {
    setData("rules", data.rules.filter((_: unknown, i: number) => i !== index));
  };
  ...
  return (
    <>
      <Head title={`Edit Grading Scale: ${scale.name}`} />
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {data.rules.map((rule: Record<string, unknown> & { grade: string; min_percentage: number; max_percentage: number; grade_point: number; description?: string }, index: number) => (
                  <div key={index} className="flex gap-4 items-end border p-4 rounded-md bg-muted/20">
                    <div className="space-y-2 flex-1">
                      <Label>Grade</Label>
                      <Input value={rule.grade} onChange={(e) => updateRule(index, 'grade', e.target.value)} />
                    </div>
                    <div className="space-y-2 flex-1">
                      <Label>Min %</Label>
              ...
              <div className="pt-6 flex justify-end gap-2">
                ...
                <Button type="submit" disabled={processing}>Update Scale</Button>
              </div>
            </CardContent>
          </Card>
        </form>

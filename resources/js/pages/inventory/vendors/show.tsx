...
import inventoryVendorApi, { type InventoryVendorLedgerResponse } from "@/lib/api/inventoryVendorApi";
...
export default function InventoryVendorShow({ id }: VendorShowProps) {
  ...
  const { data: ledgerData, isLoading } = useQuery({
    queryKey: ["inventory-vendor-ledger", id],
    queryFn: () => inventoryVendorApi.ledger(id),
  });

  const rawData: any = ledgerData?.data;
  const payload: InventoryVendorLedgerResponse | undefined = rawData?.data ?? rawData;
  ...
  const settleMutation = useMutation({
    mutationFn: (formData: typeof settleForm) => {
      return inventoryVendorApi.settle(id, {
    ...
    onSuccess: (res: any) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["inventory-vendor-ledger", id] });
  ...
  const handleExportVendorExcel = () => {
    const url = inventoryApi.purchases.exportUrl({ inventory_vendor_id: String(id) });
  ...
  if (isLoading) {
    return (
      <div className="p-8 text-center text-muted-foreground">
        Loading supplier details & ledger...
      </div>
    );
  }
    ...
    return (
      <div className="p-8 text-center text-muted-foreground">
        ...
        <Button asChild variant="outline" className="mt-4">
          <Link href="/inventory/vendors">Return to Suppliers List</Link>
        </Button>
      </div>
    );
  }
  ...
  return (
    <>
      <Head title={`${vendor.name} — Supplier Ledger`} />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground mt-1">
                ...
                {vendor.contact_email && (
                  <a href={`mailto:${vendor.contact_email}`} className="flex items-center gap-1 hover:underline text-primary">
                ...
                {vendor.bank_account_no && (
                  <span className="text-[11px] bg-muted px-1.5 py-0.5 rounded">
                    Bank: {vendor.bank_name ? `${vendor.bank_name} ` : ""}A/C: {vendor.bank_account_no}
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>Download Supplier Excel</span>
            </Button>
          ...
          <Card className="border border-border/70 shadow-sm">
            <CardContent className="p-5 flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-muted-foreground">Deferred payment purchases</p>
              </div>
          ...
          <Card className="border border-border/70 shadow-sm">
            <CardContent className="p-5 flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-muted-foreground">Paid on purchase spot</p>
              </div>
          ...
          <TabsContent value="purchases">
            <Card className="border border-border/70 shadow-sm">
              <CardHeader className="p-4 border-b bg-muted/20 flex flex-row items-center justify-between">
                <div>
                  <CardTitle className="text-base font-semibold">Purchases History</CardTitle>
                  <CardDescription className="text-xs">
                    All inventory items procured from {vendor.name}
                  </CardDescription>
                </div>
                <Button
                  size="sm"
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted/40 hover:bg-muted/40">
                      ...
                      <TableHead className="text-xs font-semibold">Bill / Invoice No</TableHead>
                      <TableHead className="text-xs font-semibold">Items Summary</TableHead>
                      ...
                      <TableHead className="text-xs font-semibold">Purchased By</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {purchases.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={7} className="h-32 text-center text-muted-foreground text-sm">
                          No purchases recorded yet from this supplier.
                        </TableCell>
                      </TableRow>
                    ) : (
                        ...
                        return (
                          <TableRow key={purchase.id} className="hover:bg-muted/30">
                            ...
                            <TableCell className="text-xs whitespace-nowrap font-medium">
                              {typeof purchase.purchased_at === "string" ? purchase.purchased_at.slice(0, 10) : "—"}
                            </TableCell>
                              ...
                              {purchase.lines && purchase.lines.length > 0 ? (
                                <div className="space-y-0.5">
                                  ...
                                  <p className="text-[11px] text-muted-foreground truncate">
                                    {purchase.lines.map((l: any) => `${l.item?.name || 'Item'} (${l.quantity} ${l.item?.unit || ''})`).join(', ')}
                                  </p>
                                </div>
                              ) : (
                                <span className="text-muted-foreground/60">—</span>
                            ...
                            <TableCell className="text-right text-xs font-bold text-foreground">
                              ₹{Number(purchase.total_cost || 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
                            </TableCell>
            ...
            <Card className="border border-border/70 shadow-sm">
              <CardHeader className="p-4 border-b bg-muted/20 flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription className="text-xs">
                    Payments made to {vendor.name} to clear credit purchase bills
                  </CardDescription>
                </div>
                {summary.outstanding > 0 && (
                ...
                <Table>
                  ...
                  <TableBody>
                    {settlements.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={7} className="h-32 text-center text-muted-foreground text-sm">
                          No settlements recorded yet for this supplier.
                        </TableCell>
                      </TableRow>
                    ) : (
          ...
          <DialogContent className="w-[95vw] sm:max-w-md max-h-[85vh] flex flex-col p-0 overflow-hidden">
            <DialogHeader className="p-4 sm:p-5 pb-3 border-b shrink-0 bg-background">
              ...
              <DialogDescription className="text-xs">
                Make a payment towards <strong>{vendor.name}</strong> to clear pending credit purchase bills.
              </DialogDescription>
            </DialogHeader>

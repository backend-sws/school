...
class FeeOverdueReminderNotification extends Notification implements ShouldQueue
{
    use NotifiesViaChannelMatrix, Queueable;

    public string $notificationType = 'fee_overdue_reminder';

    /**
     * @param  array<string, float|int|string>  $ledgerBreakdown
     */
    ...
    public function toArray(object $notifiable): array
    {
        ...
        return array_merge([
            ...
            'title' => 'Fee overdue',
            'body' => sprintf(
                'Fee for %s (period %s) was due on %s. Outstanding: ₹%s.',
                $this->student->name,
                $this->periodKey,
                $dueStr,
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $mail = (new MailMessage)
            ->subject('Fee Overdue Alert - ' . $this->student->name . ' (' . $this->periodKey . ')')
            ->greeting('Hello ' . $this->student->name . '!')
            ->error()
            ->line(sprintf(
                'Your fee for the period **%s** was due on **%s** and is now overdue.',
                $this->periodKey,
                $dueStr
            ));
        ...
        if (! empty($L)) {
            $mail->line('**Outstanding breakdown**');
            ...
            if (($L['late_fee'] ?? 0) > 0) {
                $mail->line('Late fee: **₹' . number_format((float) $L['late_fee'], 2) . '**');
            }
            if (($L['discount'] ?? 0) > 0) {
                $mail->line('Discount on structure: **₹' . number_format((float) $L['discount'], 2) . '**');
            }
            if (($L['total_payable'] ?? 0) > 0) {
                $mail->line('Total payable for this window: **₹' . number_format((float) $L['total_payable'], 2) . '**');
            }
    ...
    public function toWebPush(object $notifiable, Notification $notification): WebPushMessage
    {
        $dueStr = $this->dueDate->format('M j, Y');
        $L = $this->ledgerBreakdown;
        $line2 = sprintf('Outstanding: ₹%s', number_format($this->balanceAmount, 2));
        if (! empty($L) && ($L['total_payable'] ?? 0) > 0) {
            $line2 = sprintf(
                'Payable ₹%s · Outstanding ₹%s',
        ...
        return (new WebPushMessage)
            ->title('Fee overdue — ' . $this->periodKey)
            ->body($this->student->name . ' — was due ' . $dueStr . '. ' . $line2)
            ->data([
                'url' => '/fees/payments',
                'type' => 'fee_overdue_reminder',
                'student_id' => $this->student->id,
